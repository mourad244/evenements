...
describe("organizer workflow refinements", () => {
  beforeEach(() => {
  ...
  it("/organizer/events shows attention counts, attention cues, and row CTA labels", () => {
    organizerEventsState.data = [
      {
        id: "evt-1",
        title: "Atlas Summit",
    ...
    expect(html).toContain("Live now: 1");
    expect(html).toContain("Needs attention now");
    expect(html).toContain("Live and ready to monitor");
    expect(html).toContain("Review-only state");
    expect(html).toContain("Continue draft");
    expect(html).toContain("Open event");
  });

  it("/organizer/events/[eventId] shows organizer workflow summary and updates helper copy", () => {
    ...
    expect(html).toContain("Best next step: keep details current, then review registrations");
    expect(html).toContain("Use this page for event accuracy and the registrations view for the fastest participant-status check.");
    expect(html).toContain("Organizer updates");
    expect(html).toContain("Keep edits and state changes here, then move into registrations when you need participant review.");
  });

  it("/organizer/events/[eventId]/registrations shows workflow guide, quick-glance chips, and attention-first ordering", () => {
    organizerRegistrationsState.data = {
      ...
      registrations: [
        ...
        {
          id: "reg-attention",
          participantName: "Anas El Idrissi",
          status: "CONFIRMED",
          ticketRef: null
        }
      ]
    ...
    expect(html).toContain("Waitlisted: 1");
    expect(html).toContain("Ticket refs pending: 1");
    expect(html).toContain("Use this page for participant review");
    expect(html).toContain("Needs attention");
    expect(html).toContain("Monitor");
    expect(html).toContain("Ready");

    expect(html.indexOf("Anas El Idrissi")).toBeLessThan(html.indexOf("Youssef Idrissi"));
    expect(html.indexOf("Youssef Idrissi")).toBeLessThan(html.indexOf("Sara Bennani"));
  });
});
